...
class CommentsList extends React.Component {
  ...
  async getAllComments() {
    ...
    const {
      data: { results: commentsList },
    } = await axios.get(process.env.REACT_APP_BASEURL + "guestbook");
    this.setState({ commentsList, isCommentsListLoading: false });
  }
        ...
        {this.state.isCommentsListLoading ? (
          ...
          <ul>
            {this.state.commentsList.map((comment, i) => (
              <div className="full-w  pt-10">
                <div className="full-w bg-white rounded-2xl px-10 py-8 shadow-lg hover:shadow-2xl transition duration-500">
                  <div className="mt-4">
